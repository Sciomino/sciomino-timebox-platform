...
class passNew extends control {
	...
	$prevMail = "";
		...
		# get username
		$userId = getUserIdFromUserName($user);
		$userInfo = current(UserApiListUserWithQuery("reference=".$userId."&format=short"));
		...
		$bodyFiller['host'] = $XCOW_B['this_host']."/setting/password";
		// $body = language_template('session_mail_passnew_body', $bodyFiller);
		$body = mail_template('1.3.password', $bodyFiller, $this->ses['response']['language']);
 		sendMailWithHTML("${XCOW_B['this_name']}", "${XCOW_B['this_mail']}", $userName, $this->mail, $subject, $body);
