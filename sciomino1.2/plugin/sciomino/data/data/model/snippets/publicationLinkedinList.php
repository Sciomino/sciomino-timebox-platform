...
class publicationLinkedinList extends control {
	...
	if ($myLinkedinRef != '' && $yourLinkedinRef != '' && $myLinkedinId == $yourLinkedinId) {
		...
		$response = OauthClientGetResponse($this->id, "linkedin", "https://api.linkedin.com/v1/people/id=".$yourLinkedinId.":(first-name,last-name,headline,industry,summary,specialties,public-profile-url)", "GET", $headers, $params);
		...
		if (isset($xml)) {
			...
			if ($xml->getName() == "error") {
				...
				// don't do this anymore, only show public views as of 2015-10-05 (actually 2015-05-12)
				/*
