...
class webHome extends control {
    ...
    function Run() {
		...
		$this->max = 10;
		$activityList = array();
		$activityList = UserApiListActivityWithQuery("order=date&direction=desc&offset=".$this->offset."&limit=".$this->limit);
		...
		foreach ($activityList as $key => $activity) {
			...
			if ($activity['Title'] == "save_user") {
				$userArray = UserApiListUserWithQuery("mode=active&user[".$activity['UserId']."]");
