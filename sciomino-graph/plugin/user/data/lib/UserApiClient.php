...
//////////
function UserApiListListSave($id) {
	$response = 0;
	
	$bodyArray = UserApiDetail("index/generate?action=one&userId=".$id, array());
...
function UserApiListListDelete($id) {
	$response = 0;
	
	$bodyArray = UserApiDetail("index/generate?action=del&userId=".$id, array());
...
function UserApiListListAll($query) {
	...
	// apc cache - read
	if ($XCOW_B[use_apc] && func_num_args() > 1 && $apcName = func_get_arg(1)) {
		$apcResult = apc_fetch($XCOW_B[apc_prefix].$apcName);
		if ($apcResult) {
			return unserialize($apcResult);
		}
	}

	$bodyArray = UserApiDetail("index/listAll?", $query);
	$returnValue = UserApiListSearchResponse($bodyArray);

	// apc cache - write
	if ($XCOW_B[use_apc] && $apcName) {
		apc_store($XCOW_B[apc_prefix].$apcName, serialize($returnValue), $XCOW_B[apc_ttl]);
	}
...
function UserApiListSearchResponse($bodyArray) {
	...
	$searchList['publicList'] = array();
	$searchList['managerList'] = array();
	...
	if ($responseStatus == "200 OK") {
		...
		if ($bodyArray['Response'][0]['Content'][0]['Summary'][0]['CompleteListSize']['0'] > 0) {
			...
			if (isset($bodyArray['Response'][0]['Content'][0]['Industry'][0]['entry'])) {
				foreach ($bodyArray['Response'][0]['Content'][0]['Industry'][0]['entry'] as $entry) {
					$name = $entry['name'][0];
					$searchList['industry'][$name] = $entry['count'][0];
				}
			}
			...
			if (isset($bodyArray['Response'][0]['Content'][0]['PublicList'][0]['entry'])) {
				foreach ($bodyArray['Response'][0]['Content'][0]['PublicList'][0]['entry'] as $entry) {
					$name = $entry['name'][0];
					$searchList['publicList'][$name] = $entry['count'][0];
				}
			}

			if (isset($bodyArray['Response'][0]['Content'][0]['ManagerList'][0]['entry'])) {
				foreach ($bodyArray['Response'][0]['Content'][0]['ManagerList'][0]['entry'] as $entry) {
					$name = $entry['name'][0];
					$searchList['managerList'][$name] = $entry['count'][0];
				}
			}
...
function UserApiGetSection($name, $object, $object_id) {
	...
	if ($responseStatus == "200 OK") {
		...
		// note: assume sections do exist because they are 
		// - created in 'UserApiCreateUser' (address, contact & organization)
		// - and otherwise created in the code, file: 'ScioMino.php' (publication & experience)
		if ($listSize == 1) {
...
function UserApiGetProfile($name, $object, $object_id) {
	...
	if ($responseStatus == "200 OK") {
		...
		$listSize = $bodyArray['Response'][0]['Content'][0]['Summary'][0]['CompleteListSize'][0];
		// note: profiles are assumed to exist because
		// - the profile and its annotations are created in pairs in 'UserApiSaveProfileAnnotationList'
		if ($listSize == 1) {
...
function UserApiGetUserFromReference($reference) {
	...
	// request
	$bodyArray = UserApiList("user", '0', 'format=short&reference='.$reference);
...
function UserApiUpdateUserByReference ($user, $reference) {
	...
	if ($XCOW_B[use_apc]) {
		# delete on reference
		$toDelete = new APCIterator('user', '/^'.$XCOW_B[apc_prefix]."SC_UserApiGetUserFromReference_".$reference.'/', APC_ITER_VALUE);
		apc_delete($toDelete);
		$toDelete = new APCIterator('user', '/^'.$XCOW_B[apc_prefix]."SC_UserApiListUserWithQuery_".$reference.'/', APC_ITER_VALUE);
		apc_delete($toDelete);

		# delete on id
		$toDelete = new APCIterator('user', '/^'.$XCOW_B[apc_prefix]."SC_UserApiListUserById_".$userId.'/', APC_ITER_VALUE);
		apc_delete($toDelete);
	}
...
function UserApiListUserWithQuery($query) {
	...
	$returnValue = UserApiListUserResponse($bodyArray);
...
function UserApiListUserByReference($reference) {
	...
	$userId = UserApiGetUserFromReference($reference, "SC_UserApiGetUserFromReference_".$reference);

	return UserApiListUserById($userId, "SC_UserApiListUserById_".$userId);
...
function UserApiDeleteUser ($id) {
	$userId = 0;

	// request
	$bodyArray = UserApiDelete("user", $id);
...
function UserApiListDataById($id) {

	$bodyArray = UserApiList("data", $id, "");
...
function UserApiUpdateData ($id, $data) {
    $dataId = 0;

	// request
	$bodyArray = UserApiUpdate("data", $id, $data);
...
function UserApiSaveActivity ($activity) {
	...
	// apc cache - delete
	if ($XCOW_B[use_apc] && func_num_args() > 1 && $apcName = func_get_arg(1)) {
		$toDelete = new APCIterator('user', '/^'.$XCOW_B[apc_prefix].$apcName.'/', APC_ITER_VALUE);
		apc_delete($toDelete);
	}
...
function UserApiListActivityWithQuery($query) {
	...
	$returnValue = UserApiListActivityResponse($bodyArray);
...
function UserApiListStats() {
	...
	// apc cache - read
	if ($XCOW_B[use_apc] && func_num_args() > 0 && $apcName = func_get_arg(0)) {
		$apcResult = apc_fetch($XCOW_B[apc_prefix].$apcName);
		if ($apcResult) {
			return unserialize($apcResult);
		}
	...
	$returnValue = UserApiListStatsResponse($bodyArray);

	// apc cache - write
	if ($XCOW_B[use_apc] && $apcName) {
		apc_store($XCOW_B[apc_prefix].$apcName, serialize($returnValue), $XCOW_B[apc_ttl_long]);
	}
