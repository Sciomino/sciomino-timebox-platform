...
class statsTrend extends control {
    ...
    function Run() {
        ...
        $this->trend = $this->ses['request']['param']['trend'];
        if (! isset($this->trend)) { $this->trend = "UserCount"; }
		...
		$trendGroup = "people";
		$trendGroupList = array(
			"people" => array("UserCount", "UserKnowledgeCount", "UserHobbyCount", "UserTagCount", "UserExperienceCount", "UserPublicationCount"),
			"knowledge" => array("KnowledgeCount", "HobbyCount", "TagCount"),
			"experience" => array("ProductCount", "CompanyCount", "EducationCount", "EventCount"),
			"publication" => array("UserTwitterCount", "UserLinkedinCount", "BlogCount", "PresentationCount", "WebsiteCount", "OtherPubCount")
		);

		foreach ($trendGroupList as $key => $val) {
			foreach ($val as $item) {
				if ($this->trend == $item) {
					$trendGroup = $key;
					break 2;
		...
		if ($access) {
			...
			$dayList = array(1,4,7,10,13,16,19,22,25,28);
			$statsList = array();
		...
		$this->ses['response']['param']['appCount'] = count($myApps);
			...
			$this->ses['response']['param']['trend'] = $this->trend;
			$this->ses['response']['param']['trendGroup'] = $trendGroup;
			$this->ses['response']['param']['trendList'] = $trendGroupList[$trendGroup];
