...
class accessRuleList extends control {
	...
	#
        if (isset($this->id)) {
                ...
                $where .= "(AccessRule.AccessRuleId = \"".$this->id."\")";
        }

	# Search with searchwords in account
        if (isset($this->query)) {
                if ($where != "") { $where .= " AND "; } else { $where .= "WHERE "; }
                $where .= "(AccessRuleAttribute like \"%".safeInsert($this->query)."%\")";
        }

        if (isset($this->name)) {
                if ($where != "") { $where .= " AND "; } else { $where .= "WHERE "; }
                $where .= constructWhereWithMatch('AccessRuleAttribute', $this->name, $this->name_match);
        }
	...
	#
		...
		switch ($this->order) {
			case "id":
               			$order .= "AccessRule.AccessRuleId";
				break;
			case "name":
               			$order .= "AccessRuleAttribute";
	...
	#
        # Get List
        #
	$accessRuleList = AccessRuleListWithValues($table, $where, $order, $limit, $expand);

	#
	# Summary
	#
	$this->ses['response']['param']['listSize'] = MysqlCountWithValues("AccessRule.AccessRuleId", $table, $where, $order);
	...
	# Content
	#
	$this->ses['response']['param']['accessRuleList'] = $accessRuleList;
