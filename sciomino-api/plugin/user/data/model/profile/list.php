...
class profileList extends control {
	...
	#
		...
		foreach (array_keys($this->annotation) as $aKey) {
			...
			if ($this->annotation[$aKey]) {
				$annotationString = "(ProfileId in (SELECT DISTINCT {$annotationProperties['table']}.ProfileId FROM {$annotationProperties['table']} WHERE (AnnotationAttribute = \"".safeInsert($aKey)."\" AND AnnotationValue = \"".safeInsert($this->annotation[$aKey])."\")))";
			}
			else {
				$annotationString = "(ProfileId in (SELECT DISTINCT {$annotationProperties['table']}.ProfileId FROM {$annotationProperties['table']} WHERE AnnotationAttribute = \"".safeInsert($aKey)."\"))";
			}
	...
	#
        # Get List
        #
	$profileList = UserProfileListWithValues($table, $where, $order, $limit, $profileProperties, $expand);
