...
//Updated 20140303 to fit in our platforn
// -instead of starting on page load, we create a function with three parameters.
// -showTwitterFeed, with type (=user|search), query (=userId | searchString) and count (=number of tweets)

function showTwitterFeed(type, query, count) {
    var displaylimit = count;
	...
	// removed basic html, which is put in the view
	
	// changed to $.ajax call
	$.ajax({
		url: XCOW_B['url'] + '/snippet/publication-twitter-feed',
		dataType: 'json',
		data: {
			type: type,
			query: query
