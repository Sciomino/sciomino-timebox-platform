...
if (count($session['response']['param']['actList']) > 0) {
	...
	foreach ($session['response']['param']['actList'] as $actKey => $actVal) {
		...
		$html .= "<a class='sc_wi_link' target='_blank' href='".$XCOW_B['this_host'].$XCOW_B['url']."/act/view?act=".$actVal['Id']."'>";
		...
		$html .= "<div class='sc_wi_innerarea'>";
			...
			$html .= "<div class='sc_wi_text'>".$description."</div>";
	...
	$html .= "<a class='sc_wi_link_footer' target='_blank' href='".$XCOW_B['this_host'].$XCOW_B['url']."/act?".$session['response']['param']['actFocus']."'>".$footerText."</a>";
	$html .= "</div>";
