<div class="metoo-content">
	<div class="header">
		...
		<h2 class="divider"><?php echo language('sciomio_text_education'); ?></h2>
	</div>
	<form action="<?php echo $XCOW_B['url'] ?>/snippet/education-new-form" method="post">
        ...
        <fieldset class="details">
            <div class="inputset specs <?php if ($session['response']['param']['prevSubject'] == "" && $session['response']['param']['go'] == 1) { echo "error"; } ?>">
                <label for="subjectInput"><?php echo language('sciomio_text_education_subject'); ?></label>
                <input id="subjectInput" type="text" class="text autocomplete" data-results="<?php echo $XCOW_B['url'] ?>/snippet/suggest-local?type=education" value="<?php echo htmlTokens($session['response']['param']['prevSubject']); ?>" name="com_subject" maxlength="128" />
            </div>
            <div class="inputset specs <?php if ($session['response']['param']['prevTitle'] == "" && $session['response']['param']['go'] == 1) { echo "error"; } ?>">
                <label for="titleInput"><?php echo language('sciomio_text_education_title'); ?></label>
                <input id="titleInput" type="text" class="text autocomplete" data-results="<?php echo $XCOW_B['url'] ?>/snippet/suggest-local?type=education&subtype=" data-results-input="subjectInput" value="<?php echo htmlTokens($session['response']['param']['prevTitle']); ?>" name="com_title" maxlength="128" />
            </div>
            <div class="inputset specs <?php if ($session['response']['param']['prevPublisher'] == "" && $session['response']['param']['go'] == 1) { echo "error"; } ?>">
                <label for="type"><?php echo language('sciomio_text_education_publisher'); ?></label>
                <input id="type" type="text" class="text autocomplete" data-results="<?php echo $XCOW_B['url'] ?>/snippet/suggest-local?type=education&subtype=publisher" value="<?php echo htmlTokens($session['response']['param']['prevPublisher']); ?>" name="publisher" maxlength="128" />
            </div>
