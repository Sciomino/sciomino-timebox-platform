...
class profileList extends control {
	...
	# Search with searchwords in firstname an lastname
        if (isset($this->query)) {
                if ($where != "") { $where .= " AND "; } else { $where .= "WHERE "; }
                $where .= "(ProfileName like \"%".$this->query."%\" OR ProfileGroup like \"%".$this->query."%\")";
        }
        if (isset($this->name)) {
                if ($where != "") { $where .= " AND "; } else { $where .= "WHERE "; }
	...
	#
        # Get List
        #
	$profileList = ConnectProfileListWithValues($table, $where, $order, $limit, $profileProperties, $expand);
