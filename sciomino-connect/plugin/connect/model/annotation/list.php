...
class annotationList extends control {
	...
	# Search with searchwords in firstname an lastname
        if (isset($this->query)) {
                if ($where != "") { $where .= " AND "; } else { $where .= "WHERE "; }
                $where .= "(AnnotationAttribute like \"%".$this->query."%\" OR AnnotationValue like \"%".$this->query."%\")";
        }
        if (isset($this->name)) {
                if ($where != "") { $where .= " AND "; } else { $where .= "WHERE "; }
	...
	#
        # Get List
        #
	$annotationList = ConnectAnnotationListWithValues($table, $where, $order, $limit, $annotationProperties, $expand);
