...
#
# 6. AUTHORIZATION CHECK
# - only allow access from known frontends
#
if ($XCOW_B['auth']['on']) {
	...
	# get secret
	$authResult = mysql_query("SELECT AuthAppSecret, AuthAppSuffix FROM AuthApp Where AuthAppName like '".safeInsert($id)."'", $XCOW_B[$db]['mysql_link']);
	$authResult_row = mysql_fetch_row($authResult);
	$secret = $authResult_row[0];
	$suffix = $authResult_row[1];
	
	# check access for the required group
	$session['response']['param']['appId'] = 0; 
	$session['response']['param']['groupId'] = 0; 
	$group = $session['request']['param']['group'];
	$authResult = mysql_query("SELECT AuthApp.AuthAppId, AuthAppGroup.AuthAppGroupId FROM AuthApp, AuthAppGroup Where AuthApp.AuthAppId = AuthAppGroup.AuthAppId AND AuthApp.AuthAppName like '".safeInsert($id)."' AND AuthAppGroup.AuthAppGroupName like '".safeInsert($group)."'", $XCOW_B[$db]['mysql_link']);
	$authResult_row = mysql_fetch_row($authResult);
	if ($authResult_row[0]) {
		$session['response']['param']['appId'] = $authResult_row[0];
		$session['response']['param']['groupId'] = $authResult_row[1];
	}
	...
	if ($key != sha1($nonce.$id.$secret) || $session['response']['param']['appId'] == 0) {
...
# - access level 9 introduced for mobile resources 
# - to authorize we sent a secret out-of-band
# - next a token is required, it's the session identifier
# - to protect the token, we expect id, nonce & key (the same as we expect from api users, see [6.] )
# - it's much safer then oauth2 implicit grant, simple passing the token is not enough, so cool :-)
if ( $session['response']['access'] == 9 ) {
	# get auth params
	$token = $session['request']['param']['token'];
	$token_id = $session['request']['param']['token_id'];
	$token_nonce = $session['request']['param']['token_nonce'];
	$token_key = $session['request']['param']['token_key'];
	...
	# get secret
	$authResult = mysql_query("SELECT MobileAccessSecret, MobileAccessTimestamp, SessionId FROM MobileAccess Where MobileAccessActive = 1 AND MobileAccessEmail = '".safeInsert($token_id)."' AND MobileAccessToken = '".safeInsert($token)."'", $XCOW_B['sessionDB']['mysql_link']);
	$authResult_row = mysql_fetch_row($authResult);
	$secret = $authResult_row[0];

    $session['id'] = $authResult_row[2];
    $session['time'] = $authResult_row[1];

	# close session db
	mysql_close($XCOW_B['sessionDB']['mysql_link']);

	if ($token_key != sha1($token_nonce.$token_id.$secret)) {
		$session['response']['model']  = $XCOW_B['control']['mobile/error401']['model'];
		$session['response']['class']  = $XCOW_B['control']['mobile/error401']['class'];
		$session['response']['view']   = $XCOW_B['control']['mobile/error401']['view'];
		$session['response']['anonymous'] = $XCOW_B['control']['mobile/error401']['anonymous'];
		$session['response']['database'] = $XCOW_B['control']['mobile/error401']['database'];
		$session['response']['access'] = $XCOW_B['control']['mobile/error401']['access'];
	}
}

# - other access levels apply for default web resources
# - level 0: no session required
# - level 1: a session is required (the user is identified, but not autorized)
# - level 2: an authorized session is required (username with password)
# - level 3: is reserved for editors (but in social, everybody is an editor...)
# - therefore normal users require SessionAccessLevel = 3 in their account
# - (SessionAccessLevel 5 is reserved for administrators)
elseif ( $session['response']['access'] > 0 ) {
	...
	if ($AUTHmode == "oauth") {
		...
		$controlResult = mysql_query("SELECT UserId, OauthAccessAuthorized, OauthAccessToken, OauthAccessSecret, OauthAccessStatus, OauthAccessTimestamp FROM OauthAccess Where OauthAccessToken = '".safeInsert($Oauth['oauth_token'])."'");
	...
	#
        	...
        	if ($session['response']['anonymous']) {
        	    ...
        	    $controlResult = mysql_query("SELECT SessionAnonymousId, SessionAnonymousTimestamp FROM SessionAnonymous Where SessionAnonymousKey = '".safeInsert($controlKey)."'");
        	    $controlResult_row = mysql_fetch_row($controlResult);
		    ...
		    # echo "SESSION: ".$session['id'];
	            ...
	            $controlResult = mysql_query("SELECT SessionID, SessionAccessLevel, SessionTimestamp FROM Session Where SessionKey = '".safeInsert($controlKey)."'");
	            $controlResult_row = mysql_fetch_row($controlResult);
	...
	if ($session['response']['access'] > 1) {
		...
		if ($XCOW_B['session_remote_authentication']) {
			...
			if ($session['request']['user'] == '') {
				...
				$controlResult = mysql_query("SELECT SessionID, SessionAccessLevel, SessionUser, SessionTimestamp FROM Session Where SessionRemoteUser = '".safeInsert($session['request']['user'])."'");
		    	if ($controlResult && mysql_num_rows($controlResult) != 0) {
					...
					$controlResult = mysql_query("SELECT SessionID, SessionAccessLevel, SessionUser, SessionTimestamp FROM Session Where SessionUser = '".safeInsert($session['request']['user'])."'");
					if ($controlResult && mysql_num_rows($controlResult) != 0) {
						$controlResult_row = mysql_fetch_row($controlResult);
						$session['id'] = $controlResult_row[0];
						$session['time'] = $controlResult_row[3];
		...
		# local authentication, to the login screen...
                        ...
                        $session['response']['model']  = $XCOW_B['control']['session/login']['model'];
                        $session['response']['class']  = $XCOW_B['control']['session/login']['class'];
                        $session['response']['view']   = $XCOW_B['control']['session/login']['view'];
                        $session['response']['anonymous'] = $XCOW_B['control']['session/login']['anonymous'];
                        $session['response']['database'] = $XCOW_B['control']['session/login']['database'];
                        $session['response']['access'] = $XCOW_B['control']['session/login']['access'];
...
if ($XCOW_B['auth']['on']) {
	...
	# only if request=user/list & format=long & pageSize > 0
	if ($session['request']['path_info'] == "user/list" && $session['request']['param']['format'] == "long" && $session['response']['param']['pageSize'] > 0) {
		# open db
		$XCOW_B['mysql_link'] = mysql_connect($XCOW_B[$db]['mysql_host'], $XCOW_B[$db]['mysql_user'], $XCOW_B[$db]['mysql_pass']);
		mysql_select_db($XCOW_B[$db]['mysql_db'], $XCOW_B['mysql_link']);
		mysql_query("SET NAMES 'utf8';", $XCOW_B['mysql_link']);

		# log activity (per app)
		$activity = array();
		$activity['title'] = "availability-request";
		$activity['description'] = "App ".$session['request']['param']['availability_api_id']." requested availability for group ".$session['request']['param']['group'].". pagesize:".$session['response']['param']['pageSize'];
		$activity['priority'] = "50";
		$activity['url'] = "";
		AvailabilityActivityInsert($activity, $session['response']['param']['appId']);
		
		# log usage (per group)
		$usage = array();
		$usage['count'] = $session['response']['param']['pageSize'];
		AvailabilityUsageInsertAndUpdate($usage, $session['response']['param']['groupId']);
